...
The `jsonforms-client/` folder now mirrors the **jsonforms-react-seed** layout (Vite + React + MUI) and:
1. Fetches the JSON Schema from `GET /schema`.
2. Fetches the UI schema from `GET /ui-schema`.
3. Renders the form with JSONForms Material renderers.
4. Sends the form data to `POST /validate` for server-side validation.
...
Then open <http://localhost:5173> in your browser and click **Validate with API**.

To point the UI at a different API host, set `VITE_API_BASE` before running the dev
server, e.g. `VITE_API_BASE=http://localhost:8000 npm run dev`.
